...
void get_status(ABH3CAN *par) {
  ...
  Serial.println("***********************************************************");
  Serial.print("VelA:"); Serial.println(cnvCAN2Vel(par->DP0R.io.ayvel_fb));     //ABH3 A/Y軸速度[min^-1]の取得
  Serial.print("VelB:"); Serial.println(cnvCAN2Vel(par->DP0R.io.byvel_fb));     //ABH3 B/X軸速度[min^-1]の取得
  Serial.print("Ctrl:"); Serial.println(par->DP0R.io.control_flg, HEX);         //ABH3 制御フラグの取得
  ...
  abh3_can_reqBRD(0x29, par);                                                    //ブロードキャストパケット1(0x29)要求　*制御/ＩＯフラグの取得
  Serial.print("Input:"); Serial.println(par->BR1.io.input_flg, HEX);              //ABH3 入力フラグの取得
...
void loop() {
  float a = 1000.0;                                                             //ABH3 A/Y軸は初速度 +1000[min^-1/sec]
  float b = -1000.0;                                                            //ABH3 B/X軸は初速度 -1000[min^-1/sec]
  ...
  for (int i = 0; i < 10; i++) {
    ...
    get_status(&par);
    a -= 100.0;                                                                 //ABH3 A/Y軸速度 -100[min^-1]
    b += 100.0;                                                                 //ABH3 B/X軸速度 +100[min^-1]
    delay(1000);
  }

  abh3_can_inSet(0x00000000, 0x00000303, &par);                                //ABH3 AB軸サーボ同時OFF、スタート同時OFF　指令
  delay(3000);
}
